        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('dashboard.sms.update', $sms->id) }}" method="POST">
                        @csrf
                        @method('put')

                        <div class="mb-6">
                            <label for="identity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Identity</label>
                            <input name="identity" type="text" id="name" value="{{ old('identity') ?? $sms->identity }}" class="@error('identity') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Please enter name">
                            @error('identity')
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="sms" name="sms" rows="25" class="@error('sms') is-invalid @enderror block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Please enter message">{{ old('sms') ?? $sms->sms }}</textarea>
                            @error('sms')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
